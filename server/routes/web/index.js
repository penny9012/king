module.exports = app => {
  ...
  router.get('/news/init', async (req, res) => {
    ...
    const newsTitles = ["【520情侣皮肤】所有等待，只为重逢", "UI改造日志第四期：背包系统优化在即，局内快捷消息更智能！", "《五虎上将交响曲》揭秘，一起来看看你的音乐公开课随堂笔记吧！", "0元免流畅玩包，轻轻松松上王者", "王者荣耀联合乘车码送豪华大礼，五五开黑不氪金！", "5月27日体验服停机更新公告", "5月26日服务器优化升级公告", "5月26日全服不停机更新公告", "5月22日实战模拟维护公告", "5月22日体验服停机更新公告", "应援KPL得好礼，福利清单就绪", "DIY告白信 520陪您花式告白", "爱在峡谷 甜蜜520", "峡谷迎初夏，好礼领不停", "黄忠-烈魂五虎上将限定皮肤即将上架，缤纷好礼齐降临", "虎牙明星主播踢馆名校战队，峡谷高材生与学霸的荣耀对决", "2020年KPL春季赛常规赛最佳阵容及最佳选手评选方式公布", "2020年KPL春季赛季后赛赛程赛制公布，5月28日16:00热血开战", "【原创内容大赛音乐比赛】优秀作品合集（二）", "大众赛事合作赛道全面开启，携手合作伙伴共建王者电竞生态"]
    ...
    // 更新数据库
    await Article.deleteMany({})  //赋予一个空对象
    await Article.insertMany(newList) //放入新数据
    console.log(Article)
    res.send(newList)
  })
  ...
  router.get('/news/list', async (req, res) => {
    ...
    const cats = await Category.aggregate([
      { $match: { parent: parent._id } },
      {
        $lookup: {
          from: 'articles',  //集合名字
          localField: '_id', //本地字键
          foreignField: 'categories', //在articles里的字段
          as: 'newList'
    ...
    cats.map(cat => {
      cat.newList.map(a => {
        a.categoryName = cat.name === '热门' ? a.categories[0].name : cat.name
        return a
      })
    })
    res.send(cats)
  ...
  router.get('/articles/:id', async (req, res) => {
    const data = await Article.findById(req.params.id).lean() //加lean可以让他变成纯JSON对象
